#!/bin/bash
set -e

# Define paths to perfSONAR scripts and the original OpenSearch entrypoint
PRE_SCRIPT="/usr/lib/perfsonar/dashboards/dashboards-scripts/dashboards_secure_pre.sh"
POST_SCRIPT="/usr/lib/perfsonar/dashboards/dashboards-scripts/dashboards_secure_pos.sh"

# do not run this command
sed -i '/server.basePath/s/^/#/' "$PRE_SCRIPT"

## Set the admin password from K8S Secret
## sed -i "s/^admin .*/admin $NEW_PASS/" /etc/perfsonar/opensearch/auth_setup.out

echo "Waiting for auth file..."
while [ ! -f /etc/perfsonar/opensearch/auth_setup.out ]; do
  sleep 2
done

echo "Running pre-startup script..."
bash "$PRE_SCRIPT"

echo "Starting OpenSearch-Dashboards..."
# Drop privileges and start OpenSearch
gosu opensearch-dashboards ./opensearch-dashboards-docker-entrypoint.sh opensearch-dashboards &
OPENSEARCH_DASHBOARDS_PID=$!

# Wait for OpenSearch-Dashboards to start
sleep 10

echo "Running post-startup script..."
bash "$POST_SCRIPT"

# Re-parent OpenSearch-Dashboards to PID 1 and forward signals
trap 'kill -TERM $OPENSEARCH_DASHBOARDS_PID' TERM INT
wait $OPENSEARCH_DASHBOARDS_PID